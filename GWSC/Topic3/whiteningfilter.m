function whitenData = whiteningfilter(dataVec,psdVals,fltrOrdr,sampFreq)
%Generate a realization of stationary Gaussian noise with given 2-sided PSD
%Y = STATGAUSSNOISEGEN(Data,PSD,O,Fs)
%Generates the whiten data of the input data with a target
...
b = fir2(fltrOrdr,freqVec/(sampFreq/2),1./sqrtPSD);
...
% rng('default'); 
whitenData = sqrt(sampFreq)*fftfilt(b,dataVec);
